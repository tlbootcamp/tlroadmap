...
- Дает приток новых идей для улучшений в процессах, подходах и автоматизации
...
- Подходы и архитектура будут несогласованы. Получится Франкенштейн.
...
- Плохо настроенные процессы
  ...
  - Неизвестны критерии прохождения, можно бесконечно придираться к правкам разработчика
  - Список правок не разбит на приоритеты
...
- Для ускорения используются библиотеки и программы
  - Работают linter-ы и автоматические тесты
  - Разработчики пытаются запоминать список правок в уме
...
Можно настроить автоматическую синхронизацию комментариев в мессенджер.
...
- Можно добавить интеграцию через API, чтобы слать сообщения в мессенджер
  ...
  - Об утвержденных merge requests
...
- Вопросы "на подумать"
  - Какие плюсы дает code review в общем и вашей команде?
  ...
  - Во время code review вы увидели токсичное поведение одного из ревьюеров; например, сарказмы, грубость. Нужно ли с этим что-то делать? Если да, то что?
